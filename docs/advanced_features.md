...
Operational reference for the advanced analyzers added on schema v6. Every method states what
...
## Content-defined chunking — `analyze chunks` / `analyze chunk-overlap`  (Tier 4)
...
## Document near-duplicates — `analyze document-minhash`  (Tier 5)
...
## Format equivalence — `analyze normalized-content`  (Tiers 2–3)
...
## Cross-format derivation — `analyze derivations`  (Tier 6)
...
## Backup marginal value — `analyze backup-value`, `collections simulate-removal`
...
## Record series — `analyze record-series`
...
## Preservation risk — `analyze preservation-risk`, `preservation queue`
...
## Review prioritization & lifecycle — `analyze review-priority` / `analyze lifecycle`
...
## Photo events — `analyze photo-events`
